...
import { axiosAction, axiosActions, dateSort, notify, numberSort, showConfirm, stringSort } from '../../../../actions';
import "../../../../constants";
import { appointment_status, DELETE, GET, UPDATE } from "../../../../constants";
import { toMoment } from "../../../../utils";
import moment from 'moment';

class ReceptionPendingAppointments extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  fetchData() {
    axiosAction("/appointments-pending", GET, res => {
      this.setState({
        data: res.data,
        filterData: res.data,
        loading: false,
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record) => (record.status &&
          <span>
            {
              appointment_status.find(e => e.value == record.status).next_status.length > 0 ?
                <div className="dropdown action-label">
                  <Tag color={appointment_status.find(e => e.value == record.status).color} className="custom-badge dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                    {appointment_status.find(e => e.value == record.status).label}
                  </Tag>
                  <div className="dropdown-menu dropdown-menu-right">
                    {
                      appointment_status.find(e => e.value == record.status).next_status?.map(st => {
                        return (<a className="dropdown-item" key={st.value} onClick={() => this.onChangeStatus(record.id, st.value)}>{st.label}</a>)
                      })
                    }
                  </div>
                </div> : <Tag color={appointment_status.find(e => e.value == record.status).color} className="custom-badge">
                  {appointment_status.find(e => e.value == record.status).label}
                </Tag>
            }
          </span>
    ...
    return (
      <div className="page-wrapper">
        <div className="content">
          <div className="row">
            <div className="col-sm-4 col-3"> <h4 className="page-title">Pending Approval Appointments</h4> </div>
...
export default ReceptionPendingAppointments;
